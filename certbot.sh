...
# 复制证书和密钥到目标目录
if [[ -d "$certbot_cert_path" ]] && cp "$certbot_cert_path/fullchain.pem" "$target_cert_path" && cp "$certbot_cert_path/privkey.pem" "$target_cert_path"; then
